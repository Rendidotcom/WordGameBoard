 ...
 */
export function createBoard(containerId: string, rows = 6, cols = 5) {
  ...
  function validateWordInGrid() {
    ...
    for (let col = 0; col < cols; col++) {
      ...
      for (let row = 0; row < rows; row++) {
        const cell = grid[row * cols + col] as HTMLElement; // Pastikan casting ke HTMLElement
   ...
   */
  async function checkValidWords(words: string[]) {
